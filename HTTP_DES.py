...
class DESFileTransferApp:
    def __init__(self, root):
        self.root = root
        self.root.title("DES File Encryption/Transfer/Decryption (API with Manual URL)")
        self.root.geometry("1000x800")

        # Biến cấu hình
        self.key = "183457799B3CDFF2"  # Khóa DES mặc định
        self.host = "http://localhost:5000"  # URL mặc định (có thể thay đổi bởi người dùng)
        self.port = 5000  # Port mặc định cho Flask
    ...
    def notify_file_received(self, filename, temp_path):
        """Hiển thị thông báo hỏi người dùng có muốn nhận file hay không"""
    ...
    def start_server(self):
        ...
        flask_thread.start()
        self.log_message(f"Flask server started at http://localhost:{self.port}")
    ...
    def create_widgets(self):
        ...
        ttk.Label(config_frame, text="Host (URL):").grid(row=1, column=0, sticky="w")
        self.host_entry = ttk.Entry(config_frame, width=50)
        self.host_entry.grid(row=1, column=1, sticky="ew", padx=5)
        self.host_entry.insert(0, self.host)

        ttk.Label(config_frame, text="Port (Local):").grid(row=2, column=0, sticky="w")
    ...
    def encrypt_file(self):
        ...
        self.key = self.key_entry.get()[:8]
        if len(self.key) < 8:
            self.key = self.key.ljust(8, ' ')
    ...
    def send_file(self):
        ...
        if not self.host.startswith("http"):
            messagebox.showerror("Lỗi", "Host phải là URL hợp lệ (http:// hoặc https://)")
            return

        try:
            upload_url = f"{self.host}/upload"
