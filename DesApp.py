...
class DESFileTransferApp:
    ...
    def create_widgets(self):
        ...
        self.port_entry.bind("<KeyRelease>", self.update_receive_host)  # Cập nhật receive_host khi port thay đổi
    ...
    def encrypt_file(self):
        ...
        try:
            key_hex = self.key.encode('utf-8').hex()[:16]
    ...
    def decrypt_file(self):
        ...
        if not input_file.endswith('.enc'):
            messagebox.showwarning("Cảnh báo", "File này không có phần mở rộng .enc")
        self.key = self.key_entry.get()[:8]
        if len(self.key) < 8:
            self.key = self.key.ljust(8, ' ')
