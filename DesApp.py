...
class DESFileTransferApp:
    def __init__(self, root):
        self.root = root
        self.root.title("DES File Encryption/Transfer/Decryption (Custom DES)")
    ...
    def create_widgets(self):
        ...
        ttk.Button(action_frame, text="Nhận File", command=self.receive_file).pack(side="left", padx=5)
    ...
    # =============================================
    # CÁC HÀM DES TỰ TRIỂN KHAI (TỪ CODE TRƯỚC)
    # =============================================
    ...
    def send_file(self):
        ...
        # Cập nhật host và port từ giao diện
        ...
        try:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                try:
                    ...
                    self.log_message(f"Đã kết nối tới {self.host}:{self.port}")

                    # Gửi thông tin file
                    file_name = os.path.basename(input_file)
                    file_size = os.path.getsize(input_file)

                    # Gửi header dạng: [tên file]|[kích thước file]\n
                    ...
                    with open(input_file, 'rb') as f:
                        ...
                        while True:
                            ...
                            self.log_message(f"Đã gửi {bytes_sent}/{file_size} bytes ({progress:.1f}%)",
                                             replace_last=True)

                    elapsed = time.time() - start_time
                    speed = bytes_sent / (1024 * elapsed) if elapsed > 0 else 0
                    self.log_message(f"Đã gửi xong file {file_name} ({bytes_sent} bytes)")
                    self.log_message(f"Thời gian: {elapsed:.2f} giây ({speed:.2f} KB/s)")

                    messagebox.showinfo("Thành công", "Gửi file thành công!")
                except socket.timeout:
                    self.log_message("Lỗi: Quá thời gian kết nối")
                    messagebox.showerror("Lỗi", "Quá thời gian kết nối đến server")
                except ConnectionRefusedError:
                    self.log_message("Lỗi: Không thể kết nối - server từ chối")
                    messagebox.showerror("Lỗi", "Không thể kết nối - server từ chối hoặc không chạy")
        except Exception as e:
    ...
    def receive_file(self):
        ...
        # Tạo socket mới mỗi lần nhận file
        self.receive_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.receive_socket.settimeout(60)  # Timeout 60 giây

        try:
            self.receive_socket.bind((self.host, self.port))
            self.receive_socket.listen(1)
            self.log_message(f"Đang lắng nghe tại {self.host}:{self.port}...")

            # Sử dụng thread để không block GUI
            def accept_connection():
                try:
                    conn, addr = self.receive_socket.accept()
                    with conn:
                        conn.settimeout(30)
                        self.log_message(f"Đã kết nối với {addr}")

                        # Nhận header chứa thông tin file
                        header = b""
                        while True:
                            chunk = conn.recv(1)
                            if not chunk:
                        ...
                        try:
                            file_info = header.decode('utf-8').split("|")
                            if len(file_info) != 2:
                                raise ValueError("Header không hợp lệ")
                            file_name = file_info[0]
                            file_size = int(file_info[1])
                        except Exception as e:
                            self.log_message(f"Lỗi header: {str(e)}")
                            return
                        ...
                        if not output_file:
                            self.log_message("Đã hủy nhận file")
                            return
                        ...
                        with open(output_file, 'wb') as f:
                            ...
                            while bytes_received < file_size:
                                try:
                                    ...
                                    self.log_message(f"Đã nhận {bytes_received}/{file_size} bytes ({progress:.1f}%)",
                                                     replace_last=True)
                                except socket.timeout:
                                    self.log_message("Lỗi: Quá thời gian nhận dữ liệu")
                                    break

                        elapsed = time.time() - start_time
                        speed = bytes_received / (1024 * elapsed) if elapsed > 0 else 0
                        self.log_message(f"Đã nhận xong file {file_name} ({bytes_received} bytes)")
                        self.log_message(f"Thời gian: {elapsed:.2f} giây ({speed:.2f} KB/s)")
                        ...
                        messagebox.showinfo("Thành công", "Nhận file thành công!")
                except socket.timeout:
                    self.log_message("Hết thời gian chờ kết nối")
                except Exception as e:
                    self.log_message(f"Lỗi khi nhận file: {str(e)}")
                finally:
            ...
            self.log_message(f"Lỗi khi thiết lập kết nối: {str(e)}")
            messagebox.showerror("Lỗi", f"Không thể thiết lập kết nối: {str(e)}")
            self.receive_socket.close()
